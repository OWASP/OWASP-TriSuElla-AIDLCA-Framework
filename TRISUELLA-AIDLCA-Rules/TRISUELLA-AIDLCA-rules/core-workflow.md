...
- `.TRISUELLA-AIDLCA-rule-details/` (Platform-agnostic default — works with Cursor, VS Code, Claude Code, Windsurf, GitHub Copilot, Cline, or any AI assistant)
- `.kiro/aws-TRISUELLA-AIDLCA-rule-details/` (Kiro IDE and CLI — legacy AWS path)
- `.amazonq/aws-TRISUELLA-AIDLCA-rule-details/` (Amazon Q Developer — legacy AWS path)
...
**Conditional Enforcement**: Extensions may be conditionally enabled/disabled. See `inception/requirements-analysis.md` for the opt-in mechanism. Before enforcing any extension at ANY stage, check its `Enabled` status in `TRISUELLA-AIDLCA-docs/TRISUELLA-AIDLCA-state.md` under `## Extension Configuration`. Skip disabled extensions and log the skip in audit.md. Default to enforced if no configuration exists. 
...
- Threat Modeling (CONDITIONAL - Security-relevant scope)
...
3. Execute workspace detection:
   - Check for existing TRISUELLA-AIDLCA-state.md (resume if found)
...
- Prototype/PoC explicitly scoped out of any production deployment (document the exclusion)

**When in doubt: execute.** The cost of a missed threat far exceeds the cost of the analysis.

**Execution**:
1. **MANDATORY**: Log start of threat modeling in audit.md
2. Load all steps from `inception/threat-modeling.md`
3. Execute threat modeling:
   - Classify system type and risk tier
   - Build system context diagram (DFD)
   - Ask clarifying questions (generate and await answers)
   - Apply STRIDE + AI/Agentic threat categories (STRIDEAI) as applicable
   - Score and prioritize threats (DREAD scoring)
   - Generate threat model document
   - Derive security requirements from threat model
4. **Wait for Explicit Approval**: Present detailed completion message (see threat-modeling.md for message format) - DO NOT PROCEED until user confirms
5. **MANDATORY**: Log user's response in audit.md with complete raw input
...
**Purpose**: CI/CD pipeline generation, production deployment, observability setup, and incident response planning

**Focus**: How to DEPLOY, RUN, and OPERATE it
...
**Execute IF**:
- Project is intended for repeated deployment
- Team uses version control with pull/merge requests
...
2. Load all steps from `operations/operations.md` (CI/CD Pipeline Generation section)
3. Detect CI/CD platform (GitHub Actions, GitLab CI, etc.)
4. Generate pipeline definition with mandatory security/quality gates
5. Generate IaC security scan configuration
6. **MANDATORY**: Log completion in audit.md

---

## Deployment Planning (ALWAYS when Operations entered)

**Execution**:
1. **MANDATORY**: Log start in audit.md
2. Load all steps from `operations/operations.md` (Deployment Planning section)
3. Document deployment architecture and environment overview
4. Define deployment strategy (blue/green, canary, rolling)
5. Document rollback procedure
6. Generate production readiness checklist
...
**Execution**:
1. **MANDATORY**: Log start in audit.md
2. Load all steps from `operations/operations.md` (Observability Setup section)
3. Define metrics to monitor (application, infrastructure, security, AI/agentic if applicable)
4. Generate alerting configuration
5. Generate dashboard definition
6. **MANDATORY**: Log completion in audit.md
...
- Infrastructure Security extension is enabled
- System processes financial transactions

**Execution**:
1. **MANDATORY**: Log start in audit.md
2. Load all steps from `operations/operations.md` (Incident Response Planning section)
3. Generate incident response runbook with severity classification and escalation paths
4. Generate security-specific incident procedures (breach, credential compromise, AI abuse if applicable)
5. Generate post-mortem template
6. **Wait for Explicit Approval**: Present Operations Phase completion message - DO NOT PROCEED until user confirms
...
- **Progress Tracking**: Update TRISUELLA-AIDLCA-state.md with executed and skipped stages
...
- **Stage-Level**: Track overall workflow progress in TRISUELLA-AIDLCA-state.md
...
│
├── TRISUELLA-AIDLCA-docs/                     # 📄 DOCUMENTATION ONLY
│   ├── inception/                  # 🔵 INCEPTION PHASE
│   │   ├── plans/
│   │   ├── reverse-engineering/    # Brownfield only
│   │   ├── requirements/
│   │   ├── threat-model/           # Threat Modeling artifacts
│   │   │   ├── system-context.md
│   │   │   ├── threat-model-questions.md
│   │   │   ├── threat-model.md
...
│   │   │   │   └── network-security-design.md  # Produced when INFRA-SEC-01 active
...
│   │   └── infra-security/         # Produced when Infrastructure Security extension active
│   │       ├── hardening-baseline.md
│   │       ├── iac-scan-results.md
│   │       └── vulnerability-management-plan.md
│   ├── TRISUELLA-AIDLCA-state.md
│   └── audit.md
```

**CRITICAL RULE**:
- Application code: Workspace root (NEVER in TRISUELLA-AIDLCA-docs/)
- Documentation: TRISUELLA-AIDLCA-docs/ only
